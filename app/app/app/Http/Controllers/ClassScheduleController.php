...
class ClassScheduleController extends Controller {
		...
		if($this->panelInit->settingsArray['enableSections'] == true){
			...
			if($this->data['users']->role == "student"){
				...
				$classes = \classes::where('classAcademicYear',$this->panelInit->selectAcYear)
					->whereIn('id', MClass::getClassesIdsByParentId($this->data['users']->id))
				  ->get();
	...
	public function listTeacherwise($day_number, $page = 1){
		...
		if($query->count()) {
			...
			foreach ($data as $key => $item) {
				...
				if(isset($classes[MClass::getClassIdBySectionId($item['sectionId'])])) {
					$data[$key]['class_name'] = $classes[MClass::getClassIdBySectionId($item['sectionId'])];
				} else {
					unset($data[$key]);
				}
	...
	public function listClasswise($day_number, $page = 1){
		...
		// $break_subject = Subject::wshere('subjectTitle', 'Break')->first();
		// if($break_subject != null) {
		// 	$query = $query->where('subjectId', '!=', $break_subject->id);
		// }
					...
					foreach ($days_array as $day_number => $day_name) {
						...
						foreach ($teacher_rows as $key => $row) {
							if($row->dayOfWeek == $day_number) {
								$class_id = MClass::getClassIdBySectionId($row->sectionId);
				...
				if($check == 0) {
					...
					foreach ($all_classes as $key => $class) {
						if($class['classTeacher']) {
							if(strpos($class['classTeacher'], '"'. $teacher_id .'"') !== false) {
						    $teacher_classes[] = $class['id'];
						    $teacher_subjects = array_merge($teacher_subjects, json_decode($class['classSubjects']));
							}
						}
					}
