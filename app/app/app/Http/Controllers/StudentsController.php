	...
	public function listAll($page = 1)
	{
		...
		if($this->data['users']->role == "parent" ){
			...
			$classes_ids = \classes::where('classAcademicYear',$this->panelInit->selectAcYear)
			  ...
			  ->pluck('id');
			$sections_ids = Main::getSectionsIdsByClassTeacherId($this->data['users']->id);
		...
		foreach($students as $student){
			$toReturn['students'][] = array(
				...
				"username"=>$student['username'],"email"=>$student['email'],"isLeaderBoard"=>$student['isLeaderBoard'],
				"studentClass"=>isset($classArray[$student['studentClass']]) ? $classArray[$student['studentClass']] : "",
				"studentSection"=>isset($sectionArray[$student['studentSection']]) ? $sectionArray[$student['studentSection']] : ""
			);
		}
