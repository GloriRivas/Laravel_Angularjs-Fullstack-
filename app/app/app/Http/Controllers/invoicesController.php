		...
		if (\Input::hasFile('excelcsv')) {
			if ( $_FILES['excelcsv']['tmp_name'] ) {
				...
				if(
					...
					is_null($class_id) || empty($class_id) ||
					is_null($section_id) || empty($section_id)
				) {
				...
				$feeForInfo = array();
				$feeForInfo['class'] = $class_id;
				$feeForInfo['section'] = [$section_id];
				$feeForInfo['student_type'] = $student_type;

				$feeAllocationfeeForInfo = json_encode($feeForInfo);
				$paymentStudent = $this->panelInit->getPaymentUsers('class', $feeAllocationfeeForInfo);
