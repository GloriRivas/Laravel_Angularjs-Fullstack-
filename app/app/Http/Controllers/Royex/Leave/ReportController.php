...
use App\Repositories\Royex\LeaveRepository;
...
use App\Models2\Royex\LeaveType;
...
class ReportController extends Controller
{
    ...
    public function summaryReportDataFormat($employee_id){
        ...
        $employeeTotalLeaveDetails  = LeaveApplication::select('royex_leave_application.*',DB::raw('SUM(royex_leave_application.number_of_day) as leaveConsume'))
                                        ->where('employee_id',$employee_id)
                                        ->groupBy('royex_leave_application.leave_type_id')
                                        ->get()->toArray();
        $arrayFormat = [];
