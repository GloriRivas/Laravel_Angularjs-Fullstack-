...
use App\libraries\Royex\Enumerations\UserStatus;

use Illuminate\Support\Facades\DB;

use App\Models2\Royex\EmployeeBonus;

use Illuminate\Http\Request;

use App\Models2\Royex\BonusSetting;
...
class GenerateBonusController extends Controller
{
    ...
    public function index() {
        $results = EmployeeBonus::with('royex_employee.payGrade','royex_employee.hourlySalaries','festivalName')->orderBy('employee_bonus_id','DESC')->get();
    ...
    public function filter(Request $request) {
        ...
        $employee         = Employee::select(DB::raw('CONCAT(COALESCE(royex_employee.first_name,\'\'),\' \',COALESCE(royex_employee.last_name,\'\')) AS fullName'),'royex_employee.date_of_birth',
                                'royex_employee.gender','royex_department.department_name','royex_department.department_name','royex_pay_grade.pay_grade_name','royex_pay_grade.gross_salary','royex_employee.employee_id',
                                'royex_pay_grade.basic_salary','royex_hourly_salaries.hourly_grade','royex_hourly_salaries.hourly_rate','royex_designation.designation_name')
