...
class ExamsListController extends Controller {
	...
	public function create(){
		...
		if(\Input::has('examSchedule')){
			...
			foreach($examSchedule as $key => $value){
				...
				$examGrade=$examSchedule[$key]['finalGrade'];
			}
			$examsList->examGrade=$examGrade;
	...
	function subjectSheduleLists($id)
	{
		...
		$returnData=DB::table('exams_list')
		...
		->get();
		$items=$returnData[0]->examSchedule;
	 
		echo json_encode($returnData);
	}
	function sheduleTerms($examId){
		$data=\Input::get();
		$year=date("Y"); 
		$data['year']=$year; $data['examId']=$examId;
	 $getData=SheduleTerm::where('classId',$data['classId'])->where('examId',$examId)->where('subjectId',$data['subjectId'])->get();
	 if(count($getData)==0)
	 {
		SheduleTerm::create($data);
	 }
	 else{
		SheduleTerm::where('classId',$data['classId'])->where('examId',$examId)->where('subjectId',$data['subjectId'])->update(['allow'=>$data['allow']]);
	 }
	 $returnData=DB::table('exams_list')
	 ->leftJoin('shedule_terms', 'exams_list.id', '=', 'shedule_terms.examId')
	...
	function termSave(){
		...
		$getData=Examterms::where('classId',$data['classId'])->where('subjectId',$data['subjectId'])->get();
		if(count($getData)==0){
			Examterms::create($data);
		}
		else{
			Examterms::where('classId',$data['classId'])->where('subjectId',$data['subjectId'])->update($data);
		}
		echo 'success';
	}
	...
	// 	{
	// 		$getExams=\exams_list::where('id',$item['exam_id'])->where('sheduleTerm',1)->where('examClasses', 'like', '%' . $classId . '%')->where('examSchedule', 'like', '%' . $subjectId . '%')->first();
