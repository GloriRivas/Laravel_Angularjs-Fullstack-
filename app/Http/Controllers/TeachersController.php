...
class TeachersController extends Controller {
		...
		if(\Input::has('searchInput')){
			...
			if(is_array($searchInput)){
				...
				if(isset($searchInput['class']) AND $searchInput['class'] != "" ){
					$target_teachers = MClass::where('id', $searchInput['class'][0])->first()->classTeacher;
					$target_teachers = explode(',', $target_teachers);
				...
				if(isset($searchInput['subject']) AND $searchInput['subject'] != ""){
					$target_teachers = Subject::where('id', $searchInput['subject'][0])->first()->teacherId;
					// $target_teachers = MClass::where('classSubjects', 'LIKE', '%"'.$searchInput['subject'][0].'"%')->first()->classTeacher;
					$target_teachers = explode(',', $target_teachers);
					foreach ($target_teachers as $key => $item) {
	...
	{
		$teacherIds = MClass::where('id', $class_id)->first()->classTeacher;
		$teachers = \User::where('role','teacher')
		  ->whereIn('id', json_decode($teacherIds))
		  ->select('id', 'fullName')
		  ->get()->toArray();
	...
	public function fetch_by_class_and_section()
	{
		...
		if(!is_null($inputs['class_id']) && !empty($inputs['class_id']) && !is_null($inputs['section_id']) && !empty($inputs['section_id'])) {
			...
			$teacherIds_classes = MClass::where('id', $class_id)->first()->classTeacher;
			$teacherIds_sections = Section::where('id', $section_id)
			  ->where('classId', $class_id)
			  ->first()
			  ->classTeacherId;

			$teachers = \User::where('role','teacher')
			  ->whereIn('id', json_decode($teacherIds_classes))
			  ->whereIn('id', json_decode($teacherIds_sections))
			  ->select('id', 'fullName')
			  ->get()->toArray();
	...
	public function fetch_by_class_and_subject()
	{
		...
		if(!is_null($inputs['class_id']) && !empty($inputs['class_id']) && !is_null($inputs['subject_id']) && !empty($inputs['subject_id'])) {
			...
			$teacherIds_classes = MClass::where('id', $class_id)->first()->classTeacher;
			$teacherIds_subjects = Subject::where('id', $subject_id)->first()->teacherId;

			$teachers = \User::where('role','teacher')
			  ->whereIn('id', json_decode($teacherIds_classes))
			  ->whereIn('id', json_decode($teacherIds_subjects))
			  ->select('id', 'fullName')
			  ->get()->toArray();
