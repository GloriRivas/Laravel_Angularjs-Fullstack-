...
class StudentsController extends Controller {
	...
	public function listAll($page = 1)
	{
		...
		if($this->data['users']->role == "parent" ){
			...
			$classes_ids = \classes::where('classAcademicYear',$this->panelInit->selectAcYear)
			  ->where('classTeacher','LIKE','%"'.$this->data['users']->id.'"%')
			  ->pluck('id');
			$sections_ids = MClass::getSectionsIdsByClassTeacherId($this->data['users']->id);
	...
	function marksheet($id){
		...
		$termDatas=$subjectDatas;
		$gradeDatas=GradeLevel::All();
		for($j=1;$j<=2;$j++)
			{

			foreach($termDatas as $item)
			{
					$item['term'.$j]=DB::table('exam_marks')
					->leftJoin('exams_list', 'exam_marks.examId', '=', 'exams_list.id')
					->leftJoin('examterms', 'exams_list.id', '=', 'examterms.examId')
					->where('exam_marks.subjectId',$item['id'])
					->where('exam_marks.studentId',$id)
					->where('examterms.term',$j)
					->select('exam_marks.*','exams_list.examTitle','exams_list.examGrade')
					->get();
					$num1=0;$unitGrade=0;$unitMarks=(float)0;$examGrade=(float)0;$examMarks=(float)0;
					$grade=0; $marks=0;
					foreach($item['term'.$j] as $data)
					{
						$num1+=1;
						if($num1<=2)
						{
							$unitGrade+=$data->examGrade;
							$unitMarks+=$data->totalMarks;
						}
						else{
							$examGrade=$data->examGrade;
							$examMarks=$data->totalMarks;
						}  
						$grade=$data->examGrade; $marks=$data->totalMarks;
						if($grade!=0&&$marks!=0){
							foreach($gradeDatas as $gradeData){
								$val=$marks/$grade*100;
								if($val>=$gradeData['gradeFrom']&&$val<=$gradeData['gradeTo'])
								$data->averageGrade=$gradeData['gradeName'];
							}
						}
					}
					if($unitGrade==0||$unitMarks==0||$examGrade==0||$examMarks==0)
					{
						$item['total'.$j]='';
					}
					else if($unitGrade!=0&&$unitMarks!=0&&$examGrade!=0&&$examMarks!=0)
					{
						foreach($gradeDatas as $gradeData){
							if($j==1){
								$item['total'.$j]=($unitMarks*20)/$unitGrade+($examMarks*80)/$examGrade;
								$item['total'.$j]=round($item['total'.$j],2);
							}
							else if($j==2)
							{
								$total1=$item['total1']*0.1;
								$item['total2']=($unitMarks*20)/$unitGrade+($examMarks*70)/$examGrade+$total1;
								$item['total2']=round($item['total2'],2);
							}
							if($item['total'.$j]>=$gradeData['gradeFrom']&&$item['total'.$j]<=$gradeData['gradeTo']){
								$item['term'.$j.'Grade']=$gradeData['gradeName'];
							}
						}
					}
			}
		...
		for($i=1;$i<=2;$i++)
		{
			$getData=Examterms::where('term',$i)->get();
			foreach($getData as $item){
				$examData=ExamsList::where('id',$item['examId'])->first();
				$item['examGrade']=$examData['examGrade'];
				$item['examTitle']=$examData['examTitle'];
				$returnGradeData['unitestMarks']=$item['unitestMarks'];
				$returnGradeData['examMarks']=$item['examMarks'];
			}
			$returnGradeData['data']=$getData;

			if($i==1){$term1Grade=$returnGradeData;}
			else if($i==2){$term2Grade=$returnGradeData;}
		}

		echo json_encode(array('term1Grade'=>$term1Grade,'term2Grade'=>$term2Grade,'termDatas'=>$termDatas,'subjectDatas'=>$subjectDatas)); 
